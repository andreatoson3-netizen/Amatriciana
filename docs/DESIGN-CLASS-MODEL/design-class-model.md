...
classDiagram
   ...
   class BuildResult {
    ...
    GameController ..> CellFactory : delegates creation to
    GameController ..> CityPersistenceManager : delegates I/O to
    CellFactory ..> Cell : instantiates

    City --> CityState : owns
    CityState --> Grid : manages
    Grid --> Cell : contains
    
    %% Observer Pattern
    CityState -- "*" CityObserver : notifies
    ...
    CityState "1" *-- "1" Stats : contains

    %% Cell crea le statistiche per il calcolo del delta
    Cell ..> Stats : creates / returns

    CityState "1" --> "0..1" CityPolicyStrategy : uses
    CityPolicyStrategy <|.. EnvironmentalTax : implements
    CityPolicyStrategy <|.. IndustrialExpansion : implements
